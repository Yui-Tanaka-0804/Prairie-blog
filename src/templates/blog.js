...
export default function Blog(props) {
  ...
  const [previousSlug, nextSlug] = getSulgsAround(data.fields.slug)

  function getSulgsAround(slug) {
    const allSlugs = allBlogData.map(blog => {
      return blog.node.fields.slug
    })
    const currentSlugIndex = allSlugs.indexOf(slug);
    let previousSlug = allSlugs[currentSlugIndex - 1]
    let nextSlug = allSlugs[currentSlugIndex + 1]

    if(!previousSlug || previousSlug === '') {
      previousSlug = allSlugs[allSlugs.length - 1];
    } 

    if(!nextSlug || nextSlug === '') {
      nextSlug = allSlugs[0];
    } 
    ...
    <Layout>
      <article className={blogTemplateStyles.blog}>
        ...
        <div className={blogTemplateStyles.blog__footer}>
          <Link to={`/blog/${previousSlug}`} className={blogTemplateStyles.footer__next}>
